...
describe("GithubClient", () => {
  it("lists org teams", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse([{ slug: "platform-payments", name: "platform-payments", description: null }]));
  ...
  it("throws with status and body detail on a non-2xx response", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(new Response("team not found", { status: 404, statusText: "Not Found" }));
    vi.stubGlobal("fetch", fetchMock);
  ...
  it("respects a custom baseUrl for GitHub Enterprise Server", async () => {
    const fetchMock = vi
      .fn()
      .mockResolvedValue(jsonResponse({ login: "diego-alves", name: "Diego Alves", email: null }));
    vi.stubGlobal("fetch", fetchMock);
