...
const GENERATE_TARGETS = ["crewai", "backstage", "paperclip", "codeowners"] as const;
...
generateCommand
  .addArgument(generateCommand.createArgument("<target>", "crewai | backstage | paperclip | codeowners").choices(GENERATE_TARGETS))
  .argument("<patterns...>", "file paths, globs, or a directory to auto-discover teamapi.yml under it")
  .option("--company <name>", "company name for the paperclip target (default: \"Agent Company\")")
    ...
    .action(async (target: "crewai" | "backstage" | "paperclip" | "codeowners", patterns: string[], opts: { team?: string; out: string; company?: string; org?: string }) => {
