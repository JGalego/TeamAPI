 ...
 */
export function buildChatTools(graph: OrgGraph) {
  return [
    betaZodTool({
      name: "list_teams",
      ...
      run: async ({ type, search }) => json(listTeams(graph, { type, search }).map(toTeamSummaryDto)),
    }),
    ...
    betaZodTool({
      ...
      run: async ({ search }) => json(listServices(graph, search)),
    }),
    ...
    betaZodTool({
      ...
      run: async ({ query }) => json(searchOrg(graph, query)),
    }),
    ...
    betaZodTool({
      ...
      inputSchema: z.object({}),
      run: async () => json(orgWideCognitiveLoadReport(graph)),
    }),
  ];
}
